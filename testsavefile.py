...
#define MIN = 40
#define MAX = 85,min_cum = 3,min_rate_below = 1,max_rate_blow = 0.85
...
def show_display(img,cols,max_h,rank,list_range):
	cv.line(img,(cols-1,0),(cols - 1,max_h-1),(0,255,0),1)
	cv.putText(img, "{}".format(rank), (cols, int(max_h/2)),
				cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
	print("pos " + str(rank) + " : " + str(cols) + "---", list_range[cols])
	
def detect_ocr_words(path_img,MIN = 40,MAX = 85,min_cum = 3,min_rate_below = 1,max_rate_blow = 0.85):
...
if __name__ == "__main__":
	...
	for _idx,pos in enumerate(list_pos):
		...
		cv.imwrite('save_word/'+str(_idx)+'.jpg',crop_img)
		cv.imshow("cropped", crop_img)

	for i in range (len_list):
		path_img = 'save_word/'+str(i)+'.jpg'
		detect_ocr_words(path_img)
